...
export const ticketDetails = async (req, res) => {
    let {ticketId, groupId, deviceId, issueType, description, urgency} = req.body;
    let screenshot = req.file.filename;

    if(!ticketId || !groupId || !deviceId || !issueType || !description){
        return res.status(400).json({success: false, message: "All required fields are not filled."})
    }

    let supportData = new SupportModels({ticketId, groupId, deviceId, issueType, description, screenshot, urgency});
