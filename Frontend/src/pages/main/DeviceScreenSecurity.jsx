...
const DeviceScreenSecurity = () => {
  ...
  const getGroupData = async (currentPage, itemsPerPage, search) => {
    ...
    try {
      ...
      const response = await API.get(`/group/get-group?page=${currentPage}&limit=${itemsPerPage}&search=${search}`);
  ...
  const handleDeleteGroup = (groupId) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        try {
          loaderTimeout = setTimeout(() => setLoading(true), 1000);
          let response = await API.delete(`/group/delete-group/${groupId}`);
  ...
  return (
    <div className="main-page">
      ...
      <div className="main-page-header">
        ...
        {openGroupModal && <CreateGroup setOpenModal={setOpenGroupModal} getGroupData={getGroupData}/>}
      </div>
